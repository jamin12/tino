...
export const slideMeta: SlideMeta = {
  ...
  annotations: [
    ...
    { id: 3, label: "새 환경 생성", description: "클러스터에 등록되지 않은 커스텀 환경(canary, demo, sandbox 등)을 새로 만들어 추가할 수 있습니다." },
    { id: 4, label: "생성될 리소스 미리보기", description: "선택한 환경에 따라 k8s-app-init 레포에 실제로 생성될 파일 경로를 보여줍니다. sample 템플릿의 내용에 따라 파일 목록이 달라지며, 공통적으로 overlays/{env}/{ns}-{env}/ 폴더(sample-{env}/ 복제)가 생성되고, 해당 환경의 overlays/{env}/kustomization.yaml에 참조가 추가됩니다." },
  ],
};
      ...
      <ContentSection>
        <div className="max-w-[640px]">
          <div className="flex flex-col gap-5 bg-white rounded-lg border border-[#f0f0f0] shadow-[0px_0px_8px_#00000014] p-6">
            ...
            <div className="flex flex-col gap-2" data-annotation-id="3">
              ...
              <InfoRow label="환경 이름" labelWidth="86px">
                <div className="flex items-center gap-2 flex-1">
                  <TextInput
                    placeholder="예: canary, demo, sandbox"
                    className="flex-1"
                  />
            ...
            {/* 미리보기 */}
            <div className="border-t border-[#f0f0f0]" />
            <div className="flex flex-col gap-2" data-annotation-id="4">
              <TextCell bold color="#111" className="text-[13px]">
                생성될 리소스
              </TextCell>
