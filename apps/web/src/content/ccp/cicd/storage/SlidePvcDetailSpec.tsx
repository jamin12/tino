...
export const slideMeta: SlideMeta = {
  screenId: "CCP-STR-003-D1",
  title: "PVC 구성 - Spec",
  section: "CI/CD 저장소",
  links: [
    { targetScreenId: "CCP-STR-003-D2", type: "tab", label: "Status 탭" },
  ],
  annotations: [
    ...
    { id: 11, label: "구성 탭 전환", description: "기본정보, Spec, Status 하위 탭으로 구성 정보를 분류하여 탐색합니다. 현재 Spec 탭이 활성화되어 있습니다." },
    { id: 12, label: "Spec 정보", description: "PVC의 요청 용량, Access Modes, Storage Class, Volume Mode, Volume Name 등 스펙 사양을 표시합니다." },
  ],
};
...
export default function SlidePvcDetailSpec() {
  return (
    <CcpDashboardLayout
      breadcrumbs={[{ label: "저장소" }, { label: "PVC", isBold: true }]}
      title="Persistent Volume Claims"
      sideMenuItems={createSideMenuItems({ activeId: "cicd", activeLabel: "PVC" })}
    >
      <ListDetailLayout
        detail={
          <ResourceDetailPanel
          ...
          >
            ...
            <div className="px-5" data-annotation-id="11">
              <Tabs items={configTabs} activeId="spec" />
            </div>
            ...
            <div className="flex-1 px-4 pt-4 pb-4" data-annotation-id="12">
              <div className="bg-white rounded-sm border border-[#f0f0f0] shadow-[0px_0px_8px_#00000014] px-5 py-4 flex flex-col gap-3">
                ...
                <InfoRow label="Storage Class">
                  <TextCell color="#6d7073" className="text-[13px]">local-storage</TextCell>
                </InfoRow>
                <InfoRow label="Volume Mode">
                  <TextCell color="#6d7073" className="text-[13px]">Filesystem</TextCell>
                </InfoRow>
                <InfoRow label="Volume Name">
                  <TextCell color="#6d7073" className="text-[13px]">pvc-1a2b3c4d</TextCell>
                </InfoRow>
              </div>
            </div>
          </ResourceDetailPanel>
