...
export const slideMeta: SlideMeta = {
  ...
  annotations: [
    ...
    { id: 10, label: "구성 탭 전환", description: "기본정보, 설정, Data 하위 탭으로 구성 정보를 분류하여 탐색합니다. 현재 설정 탭이 활성화되어 있습니다." },
    { id: 11, label: "Labels / Annotations 설정", description: "ConfigMap에 할당된 Labels(app, env, managed-by)와 Annotations(ArgoCD 옵션, 설명 등)를 표시합니다. Labels는 배지 형태로, Annotations는 텍스트로 나열됩니다." },
  ],
};
...
export default function SlideConfigMapsDetailConfigSettings() {
  return (
    <CcpDashboardLayout
        ...
        detail={
          ...
          >
            ...
            <div className="px-5" data-annotation-id="10">
              <Tabs items={configTabs} activeId="settings" />
            </div>
            ...
            <div className="flex-1 px-4 pt-4 pb-4" data-annotation-id="11">
              <div className="bg-white rounded-sm border border-[#f0f0f0] shadow-[0px_0px_8px_#00000014] px-5 py-4 flex flex-col gap-3">
                <InfoRow label="Labels">
                  <Badge variant="neutral" size="sm">app: backend</Badge>
                  <Badge variant="neutral" size="sm">env: production</Badge>
                  <Badge variant="neutral" size="sm">managed-by: cone-chain</Badge>
                </InfoRow>
                <Divider />
                <InfoRow label="Annotations">
                  <TextCell color="#6d7073" className="text-[12px] break-all">
