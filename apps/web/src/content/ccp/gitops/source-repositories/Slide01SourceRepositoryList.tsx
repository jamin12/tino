...
export const slideMeta: SlideMeta = {
  ...
  annotations: [
    ...
    },
    {
      ...
      label: "삭제 확인 모달",
      description:
        "저장소 삭제 실행 전 최종 확인 대화상자입니다. 저장소 이름을 정확히 입력해야 '삭제' 버튼이 활성화됩니다. 이름이 일치하지 않으면 삭제할 수 없습니다.",
    },
  ],
};
...
export default function Slide01SourceRepositoryList() {
  return (
    <CcpDashboardLayout
      breadcrumbs={[
        { label: "홈" },
    ...
    >
      ...
      <Overlay position="fixed" top={0} left={0}>
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div data-annotation-id="8" className="bg-white rounded-lg shadow-xl w-[480px]">
            <div className="px-6 py-4 border-b border-[#e0e0e0] flex items-center justify-between">
              ...
              <button type="button" className="text-[#999] hover:text-[#333]">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="px-6 py-5 space-y-4">
              <p className="text-[13px] text-[#555] leading-relaxed">
                이 작업은 <span className="font-semibold text-[#da1e28]">되돌릴 수 없습니다</span>. 저장소와 모든 관련 데이터(코드, 커밋 이력 등)가 영구적으로 삭제됩니다.
              </p>
              <p className="text-[13px] text-[#555] leading-relaxed">
                확인을 위해 저장소 이름 <span className="font-semibold text-[#333]">app1-backend</span>을 입력하세요.
              </p>
              <input
                type="text"
                placeholder="저장소 이름 입력"
                className="w-full border border-[#ddd] rounded px-3 py-2 text-[13px] text-[#333] bg-white focus:border-[#da1e28] focus:outline-none"
              />
            </div>
