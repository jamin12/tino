...
export const slideMeta: SlideMeta = {
  ...
  annotations: [
    ...
    },
    {
      ...
      label: "팀 목록 패널",
      description:
        "기존 팀 목록을 좌측에 표시합니다. 검색과 새 팀 추가 버튼을 포함하며, 현재 새 팀 폼이 우측에 열려 있습니다.",
    },
    {
      id: 3,
      label: "뒤로 가기",
      description:
        "새 팀 추가 폼을 닫고 팀 목록 뷰로 돌아갑니다.",
    },
    {
      id: 4,
      label: "팀 이름 입력",
      description:
        "새 팀의 이름을 입력합니다. 필수 입력 항목이며, 짧고 기억하기 쉬운 이름을 권장합니다.",
    },
    {
      id: 5,
      label: "설명 입력",
      description:
        "팀의 목적이나 역할을 설명합니다. 선택 입력 항목입니다.",
    },
    {
      id: 6,
      label: "접근 설정",
      description:
        "팀의 저장소 접근 범위를 설정합니다. '특정 저장소'는 명시적으로 추가된 저장소만, '전체 저장소'는 모든 저장소에 접근을 허용합니다. 저장소 생성 권한도 별도로 설정할 수 있습니다.",
    },
    {
      id: 7,
      label: "권한 설정",
      description:
        "팀 멤버의 권한 수준을 설정합니다. '일반 접근'은 아래 권한 테이블로 세부 제어하며, '관리자 액세스'는 풀/푸시 및 공동 작업자 추가 권한을 부여합니다.",
    },
    {
      id: 8,
      label: "저장소 섹션 접근 허용 테이블",
      description:
        "코드, 이슈, 풀 리퀘스트 등 각 저장소 섹션별로 접근 불가/읽기/쓰기 권한을 개별 설정합니다. '일반 접근' 권한 선택 시 이 테이블의 설정이 적용됩니다.",
    },
    {
      id: 9,
      label: "취소 / 생성 버튼",
      description:
        "취소 버튼은 팀 생성을 중단하고 이전 화면으로 돌아갑니다. 생성 버튼은 입력된 정보로 새 팀을 생성합니다.",
    },
  ],
};
...
function TeamListItem({ team }: { team: TeamRow }) {
  return (
    <div className="w-full flex items-center gap-4 px-4 py-3.5 rounded-lg text-left border border-[#eee] bg-white hover:bg-[#f8f9fb] hover:border-[#d8dde3] transition-all">
      ...
      <div className="flex-1 min-w-0">
        <span className="text-[13px] font-bold text-[#222] truncate block">{team.name}</span>
...
const defaultPermissions: PermissionUnit[] = [
  { id: "code", label: "코드", description: "소스 코드 접근 파일, 커밋 그리고 브랜치", value: "read" },
  { id: "issues", label: "이슈", description: "버그 리포트, 작업 및 마일스톤 관리.", value: "read" },
  { id: "pulls", label: "풀 리퀘스트", description: "풀 리퀘스트 및 코드 리뷰 활성화.", value: "read" },
  ...
  { id: "packages", label: "패키지", description: "저장소 패키지 관리.", value: "read" },
...
export default function Slide04RepositoryGroupCreateTeam() {
        ...
        <div className="flex gap-5 items-start">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-[15px] font-bold text-[#111]">새 팀 추가</h3>
            </div>
            ...
            <div className="bg-white rounded-sm shadow-[0px_0px_8px_rgba(0,0,0,0.08)] px-5 py-4 flex flex-col gap-5">
              ...
              <div className="flex items-center justify-end gap-1 pt-2" data-annotation-id="9">
                ...
                <Button variant="primary" size="md">생성</Button>
              </div>
            </div>
          </div>
        </div>
